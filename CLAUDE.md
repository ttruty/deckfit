...
src/app/
  core/
    ...
    tour/               # §12a first-run guide (steps, dialog, service)
    challenges/         # §7b async challenges: gateway (Supabase | memory) + service
  ...
  domain/
    ...
    challenges/         # §7b pure rules: standings, pot, progress from sessions
...
supabase/migrations/    # the only server-side state in the app (§7b); run once per project
...
### 7b. Async challenges (implemented)

A room is live and holds nothing. A **challenge** runs for days: everyone puts reps on the line
per day, the pot grows with every missed day, and when it ends the people who missed share the
pot as work to do. `/challenges` lists yours and starts or joins one; `/challenges/:code` is the
board.

- **The deal** (`domain/models/challenge.schema.ts`): a goal — `streak` (a workout every day),
  `daily` (N points every day) or `total` (N points before it ends) — an `ante` in reps per day,
  and a start/end day. Points are the card-value scale: 1 per rep, 1 per 5 seconds (§6.1).
- **Standings** (`domain/challenges/standings.ts`, pure): a day only counts against you once it
  is over, so today is always still open (same rule as §9e). Each missed day drops that player's
  ante into the pot; a `total` goal stakes nothing until the last day, then the whole thing if
  they came up short. When it's over, everyone who missed splits the pot evenly — a lone misser
  works off exactly what they dropped.
- **Progress** (`domain/challenges/progress.ts`, pure): a device reports **only its own days**,
  taken from its saved sessions (`sessionWork` — completed tasks, rest never counts), and only
  the days whose numbers changed. Days are the player's own local calendar days. A debt is paid
  down by any work logged after the challenge ended (`debtRemaining`).
- **Storage** — the app's only server-side state: `supabase/migrations/0001_challenges.sql`
  creates `challenges`, `challenge_members` and `challenge_days` with RLS. It must be run once
...
- **Trust model**: the same as §7, and no stronger. The device id travels in an `x-device-id`
  ...
  word. Anyone with the six-character code can read and join. To make it real, turn on Supabase
  anonymous auth and swap `device_id()` for `auth.uid()`.
- **On Home**: a "Challenges" section appears when this device is in any (§8). It costs one
  request: `myChallenges()` lists them and works out where you stand with `ownStatus` — the same
  rules as the shared standings, from the sessions already on the device. Each line says what
  today wants ("Today still open · 20 reps on the line", "Today's done · 3 days to go"), and
  one that just finished says what it cost you. Order: today undone, today done, upcoming,
  recently finished (a week, then it drops off). The section is its own resource, so a slow or
  broken backend never holds Home up — it just shows nothing.
- **Reporting**: `ChallengeService.syncAll()` runs when a workout is finalised (`PlayStore`) and
  when the challenges screens open; the detail screen also subscribes to `postgres_changes`, so
...
| Route | Screen |
|---|---|
| `/` | Home: favorite routines (one-tap start), Quick Start, your challenges (§7b), start or join a room |
...
### 9d. Screen furniture (implemented)

Every screen is drawn the same way, so the app reads as one thing. The classes and the surface
tiers are global (`src/styles.scss`, not component styles, so any feature can use them).

**Three surface tiers**, so a card reads as a card. Use these, never the raw `--mat-sys-surface-*`
tokens, and the whole app re-tints from one place:

| Token | What it is |
|---|---|
| `--df-page-bg` | the backdrop — tinted in light, darkest in dark; `body` uses it |
| `--df-card-bg` | a section, and the app's chrome (toolbar, bottom nav) |
| `--df-row-bg` / `--df-row-raised` / `--df-row-hover` | an item inside a section, and its states |
| `--df-edge`, `--df-shadow` | the hairline and soft shadow that lift a card off the page |

Light lifts with white, dark lifts with lighter greys, the way M3 elevation works.

- `.df-page` — the screen: a centred column, `gap: 16px`, `h1` in the display face (§9a).
- `.df-section` — a card with its own heading (`h2`, display face): one per idea ("Favorites",
  "Built-in decks", "Look"). Rows inside it use `--df-row-bg`, so they read as items in a group.
- `.df-row` — that row treatment, for markup that wants it directly.
- `.df-head` — a heading with the action that belongs to it, wrapping on narrow screens.
- `.df-hint` — the one muted line under a heading.

Home, Library, Decks, Games, History, Settings, the lobby and the room table use them; feature
layout lives inside a section, and no screen re-invents the container. Home's Quick Start hero is
the one section that isn't a plain surface — it leads the screen. Headings borrow the deck's
display face, which is what ties the app to the cards.

### 9e. Activity grid (implemented)

History opens with "Your streak": one square per day for the last 26 weeks, shaded by the work
that day held, plus days-in-a-row, best ever and days worked.

- `activity-grid.ts` is pure and takes `today` from the injectable `Clock`, so tests are
  deterministic. Days are **local** calendar days (an 11pm workout belongs to that day), stepped
  with `setDate` rather than 86.4M ms, so a DST change can't shift the grid.
- A day's work is the card-value scale (1 per rep, 1 per 5 seconds, §6.1); shading is relative
  to the busiest day in view, so a quiet half-year still reads.
- The current streak counts back from today, and **today counts as still open** — it only breaks
  once a day has passed with nothing in it. The best streak looks at all history, not just the
  window.
- Colour is the only thing carrying the shading, so every square names its day and work in its
  `aria-label`/`title`, and the summary line says the streaks in words (§9 a11y).
- **Every square is a button**: it opens that day under the grid (`df-day-detail`) — the workouts
  in it (time, game, deck, work, length, and who else was in a room game) and what they added up
  to per exercise. Picking the same square again, or Close, shuts it; a day with nothing in it
  says so. `day-detail.ts` is pure, over the rows History already loaded.
- `dayKey` lives in `domain/challenges/progress.ts` and is shared with §7b, so a day means the
...
`core/tour`: five plain-language steps (`tour-steps.ts` — what a card asks of you, decks and
games, Quick Start, rooms, making your own) in a dialog with Back / Next / Skip and a suit-
coloured badge per step, so the guide looks like the cards.

- It opens once, on the first launch **after** the safety notice (`app.ts` chains them, so the
  notice is never buried), and never again by itself: `meta.tourSeenAt` records any close.
- Settings → Guide replays it ("Show the guide") and turns the first-run offer off
  (`meta.tourEnabled`; switching it back on re-arms it for the next launch).
- Keep the steps short and original (§1 content rule); `tour.service.spec.ts` checks the
  open-once/off/replay rules and that the steps stay short and distinct. `e2e/settings.spec.ts`
  walks it on a fresh device and from Settings; `e2e/a11y.spec.ts` scans it in both themes.
- Specs that aren't about it skip it: `newDevice()` waves it away (scoped to `df-tour`, since
  "Skip" is also a button on the task panel).
...
- Whether to add optional sign-in for cross-device sync of decks/routines. (§7b now has tables
