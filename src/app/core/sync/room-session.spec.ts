...
const summary = (v: LobbyView) => v.players.map((p) => `${p.name}${p.isHost ? '*' : ''}:${p.ready ? 'ready' : 'waiting'}`);
...
describe('RoomSession (3 loopback peers)', () => {
  it('everyone sees the same code, host, routine preview, and presence in seat order', async () => {
    ...
    for (const s of all) {
      ...
      expect(v.routine?.preview).toMatchObject({ gameName: 'Interval Deck', deckName: 'Bodyweight deck', players: { min: 1, max: 6 } });
      expect(v.players.map((p) => [p.name, p.seat, p.isHost])).toEqual([['Ann', 0, true], ['Bo', 1, false], ['Cy', 2, false]]);
  ...
  it('ready toggles propagate through the host, and the start blocker clears only when all are ready', async () => {
    ...
    b.setReady(true);
    await flush();
    for (const s of all) expect(summary(view(s))).toEqual(['Ann*:waiting', 'Bo:ready', 'Cy:waiting']);
    host.setReady(true);
    c.setReady(true);
    await flush();
  ...
  it('a player leaving drops their seat and ready flag; newcomers join not ready', async () => {
    ...
    const d = await RoomSession.join(new LoopbackTransport(hub), host.code, { id: 'di', name: 'Di' });
    await flush();
  ...
  it('rejoining with the same id keeps the seat', async () => {
    ...
    await flush();
    expect(view(host).players.map((p) => [p.name, p.seat])).toEqual([['Ann', 0], ['Bo again', 1], ['Cy', 2]]);
  ...
  it('ignores room-state and start messages that do not come from the host', async () => {
    ...
    const forged = { code: host.code, hostId: 'cy', epoch: 0, seats: ['ann', 'bo', 'cy'], phase: 'playing' as const, routine: null, ready: {} };
  ...
  it('host start reaches every peer with an identical payload, and the room enters playing', async () => {
    ...
    for (const s of all) s.setReady(true);
    await flush();
    const received = Promise.all([b, c].map((s) => new Promise<GameStart>((resolve) => s.onStart(resolve))));
  ...
  it('joining a missing room fails', async () => {
    await expect(RoomSession.join(new LoopbackTransport(new LoopbackHub()), 'ZZZZZZ', bo)).rejects.toThrow(/not found/);
  });
});
